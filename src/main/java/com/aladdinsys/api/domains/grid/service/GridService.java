...
public class GridService {
  ...
  public GridResponseDto findById(final Long id) {
    return GridResponseDto.of(
        presetRepository.findById(id).orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND)));
  }

  @Transactional(readOnly = true)
  public List<GridResponseDto> findAll() {
    return presetRepository.findAll().stream().map(GridResponseDto::of).toList();
  }
  ...
  public Long save(GridRequestDto dto) {
    ...
    GridPreset preset =
        GridPreset.builder().name(dto.name()).cols(dto.cols()).rows(dto.rows()).build();
